...
#proof[
  ...
  $
    "原式"
    &= lim_(x->+oo) (n x^(n-1)) / (lambda e^(lambda x)) = lim_(x->+oo) (n(
      n-1
    ) x^(n-2)) / (lambda^2 e^(lambda x)) = dots.c = lim_(x->+oo) (n!) / (lambda^n e^(lambda x)) = 0
  $
...
$
  f(x) = f(x_0) + f'(x_0)(x-x_0) + (f''(x_0)) / (2!) (x-x_0)^2 + dots.c + (f^((n))(x_0)) / (n!) (x-x_0)^n + R_n (
    x
  ) quad ①
$
...
$
  f(x) = f(0) + f'(0) x + (f''(0)) / (2!) x^2 + dots.c + (f^((n))(x)) / (n!) x^n + (f^((n+1))(theta x)) / ((
    n+1
  )!) x^(n+1)
$
...
$
  (1+x)^alpha = 1 + alpha x + (alpha(alpha-1)) / (2!) x^2 + dots.c + (alpha(alpha-1)dots.c(alpha-n+1)) / (n!) x^n + R_n(
    x
  ) quad (x> -1)
$
