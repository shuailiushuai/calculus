...
#example[
  ...
  #solution(tag: "证明")[
    ...
    $
      sum_(k=n+1)^(n+p) 1 / (k^2) < sum_(k=n+1)^(n+p) 1 / ((k-1)k) = sum_(k=n+1)^(n+p) (
        1 / (k-1) - 1 / k
      ) = 1 / n - 1 / (n+p) < 1 / n
    $
...
#example[
  ...
  #solution[
    ...
    $
      => arctan x = arctan 0 + int_0^x 1 / (1+x^2) dif x = x - x^3 / 3+x^5 / 5-x^7 / 7+dots.c+(
        -1
      )^n (x^(2n+1)) / (2n+1) + dots.c,space abs(x)<1
    $
  ]
]
