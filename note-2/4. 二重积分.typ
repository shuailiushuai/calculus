...
#theorem(name: [二重积分的线性性质])[
	...
	(3) 区域可加性：设 $sigma=sigma_1 union sigma_2$ 且 $sigma_1,sigma_2$ 无公共内点，则有 $iintsg f(x,y) dsg = iintb(sigma_1) f(x,y) dsg + iintb(sigma_2) f(x,y) dsg$。

	(4) 保序性：若 $f(x,y) >= 0 space ((x,y) in sigma)$，则 $iintsg f(x,y) dsg >=0$。
]